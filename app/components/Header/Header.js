...
function Header(props) {
  ...
  //hiding project structure id
  //const projectStr = JSON.parse(localStorage.getItem("direct_loading")).tarPage;
  // const comp_id = JSON.parse(localStorage.getItem("company")).fkCompanyId;
  // const project_id = JSON.parse(localStorage.getItem("projectName")).projectName
  //   .projectId;
  // const control = "controltowerbi";
  // const res = api.get(
  //   "/api/v1/" +
  //     control +
  //     `/?company=${comp_id}&project=${project_id}&portfolio=HSE`
  // );
  ...
  const fetchPhaseData = async () => {
    ...
    for (let i = 0; i < projectListData.length; i++) {
      if (
        projectListData[i].breakdown &&
        projectListData[i].breakdown.length > 0 &&
        projectListData[i].breakdown[0].structure &&
        projectListData[i].breakdown[0].structure[0].url
      ) {
        const config = {
          method: "get",
  ...
  const handleBreakdown = async (e, index, label) => {
    ...
    localStorage.setItem("selectBreakDown", JSON.stringify(tempArray));
  ...
  const fetchCallBack = async () => {
    // setSelectBreakDown([])
    try {
      const labellist = projectData.projectName.breakdown.map((item) => ({
      ...
      for (const key in projectData.projectName.breakdown) {
        if (key == 0) {
          const config = {
            method: "get",
            url: `${SSO_URL}/${
        ...
        <div className={classes.headerProperties}>
          <div className={classes.projectSwitcher}>
            ...
            >
              <DialogTitle
                onClose={handleProjectClose}
                className={classesm.projecDialogHeadTitle}
              >
                <svg
                ...
                {isLoading
                  ? labelList.map((item, index) => (
                      <Chip
                        size="small"
                        label={
                          breakDownData && breakDownData[index]
                            ? breakDownData[index].name
                            : `All-${item.breakdownLabel}`
                        }
                        key={index}
                      />
                    ))
