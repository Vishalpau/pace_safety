...
function Comments() {
  ...
  const getComments = async () => {
    api
      ...
      .then((res) => {
        console.log(res, "dgkldfgjdfgdfgjd");
        const data = res.data.data.results.map((ad) => {
          if (ad.parent) {
            return {
              ...ad,
              childPresent: true,
  ...
  const SingleCardData = ({ item, index }) => {
    ...
    const [replyEditState, setReplyEditState] = useState(item.replies && item.replie);
