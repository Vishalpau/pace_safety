...
function Comments() {
  ...
  const [childPresentData, setChildPresentData] = useState([]);
  ...
  const getComments = async () => {
    api
      ...
      .then((res) => {
        console.log(res.data.data,'dgkldfgjdfgdfgjd')
        const data = res.data.data.results.results.map(ad => {
          if (ad.parent) {
            return ({
              ...ad,
              childPresent: true
  ...
  useEffect(() => {
    console.log(childPresentData, 'childPresentData');
  },[childPresentData])
      ...
      <Grid container spacing={3}>
        <Grid item md={3} sm={4} xs={12} className="commentUserDetail">
          <Typography variant="h6" className="commentUserTitle" align="right">
            <img className="comment-image" src={item.avatar} /> {item.username}
          </Typography>
          <Typography
            variant="h6"
