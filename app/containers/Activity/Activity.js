...
function ActivityHistory(props) {
  ...
  const handleNewAhaPush = async () => {
    history.push("/app/pages/aha/assessments");
  };
  const handleNewAhaPushSummary = async () => {
    history.push("/app/pages/assesments/flhasummary");
  };
  ...
  const fetchActivityHistoryData = () => {
    // incidents
    api
      .get(`api/v1/core/useractivities/${props.module}/123/`)
    ...
    <Box marginTop={1} marginLeft={1} marginRight={1}>
                  ...
                  <TimelineItem key={key}>
                    <TimelineOppositeContent className={classes.maxWdtwohun}>
                      <Typography color="textSecondary">
                        {DateFormat(activity.dateTime, true)}
                      </Typography>
                    ...
                    <TimelineContent>
                      ...
                      <Typography color="textSecondary">
                        {activity.action}|{activity.displayMessage}
                      </Typography>
                    </TimelineContent>
                  </TimelineItem>
                ))}
      ...
      <Grid item md={12} xs={12}>
        <Pagination
          ...
          variant="outlined"
          shape="rounded"
          className={classes.mTopThirtybtten}
        />
      </Grid>
    </Box>
    // </PapperBlock>
