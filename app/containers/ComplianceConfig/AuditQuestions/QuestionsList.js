    ...
    options: {
        ...
        customBodyRender: (value,tableMeta, updateValue) => (
          ...
          {console.log(tableMeta)}
            <IconButton size="small" color="primary" className='tableActionIcons' onClick={(e) => handleClickOpen(tableMeta.rowData)}>
                <MoreVertIcon />
            </IconButton>
            <IconButton size="small" color="primary" className='tableActionIcons' onClick={() => handleClickDeleteAlert(tableMeta.rowData)}>
              <DeleteIcon />
            </IconButton>
          </>
        )
...
//   const [viewQuestion, setViewQuestion] = useState(false);
  ...
  const handleNewComplianceConfigQPush = async () => {
    ...
    history.push(
      '/app/compliance-config/question-group'
    );
  };
  ...
  const handleQuestionEditPush = async () => {
    history.push(
      `/app/compliance-config/edit/${auditId}`
    );
  };

  const handleQuestionViewPush = async () => {
    history.push(
      `/app/compliance-config/view/${auditId}`
    );
  };
  ...
  const handleDelete = async () => {
    console.log(data)
    data['status'] = 'Delete'
    const res = await api.put(`/api/v1/configaudits/auditquestions/${data.id}/?company=${data.fkCompanyId}&project=${data.fkProjectId}&projectStructure=${data.fkProjectStructureIds}`,data).then(response => {fetchAuditData() ,setDeleteQ(false); }).catch(error => {console.log(error)});
  }
    ...
    <>
      <Grid container spacing={3}>
        <Grid item md={12} sm={12} xs={12} align='right' className="paddBRemove">
          <Tooltip title="New">
              <Button size="medium" className="marginR5" variant="contained" color="primary" onClick={(e) => handleNewComplianceConfigQPush(e)}>
            ...
            <Grid container spacing={3}>
              ...
              >
                <FormControl component="fieldset">
                  <FormLabel component="legend" className="checkRadioLabel">Are you sure?</FormLabel>
                  <FormLabel component="legend" className="checkRadioLabel">You want to delete this question</FormLabel>
                </FormControl>
              </Grid>
