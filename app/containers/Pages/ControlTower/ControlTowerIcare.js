...
const ControlTowerIcare = () => {
  ...
  const load = async () => {
    ...
    const project_id = JSON.parse(localStorage.getItem('projectName'))
  ...
  const typOptions = ['Risk', 'Comment', 'Positive behaviour'];
  ...
  const isAllSelected = typOptions.length > 0 && selected.length === typOptions.length;
  categoryOptions.length > 0 && catrySelected.length === categoryOptions.length;
  classifiOptions.length > 0
    && classifSelected.length === classifiOptions.length;
  ...
  const handleChangeCategory = (event) => {
    ...
    if (value[value.length - 1] === 'all') {
      setCatrySelected(
        catrySelected.length === categoryOptions.length ? [] : categoryOptions
      );
      return;
    }
  ...
  const handleChangeClassifi = (event) => {
    ...
    if (value[value.length - 1] === 'all') {
      setClassifSelected(
        classifSelected.length === classifiOptions.length ? [] : classifiOptions
      );
      return;
    }
  ...
  const handleChangeComp = (event) => {
    ...
    if (value[value.length - 1] === 'all') {
      setCompSelected(
        compSelected.length === compOptions.length ? [] : compOptions
      );
      return;
    }
  ...
  return isLoading ? (
    ...
    'Control Tower is not configured yet.'
  ) : (
    <>
