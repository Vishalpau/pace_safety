...
const JobDetails = (props) => {
    ...
    if (Teamform.length > 1) {
      if (Teamform[index].id !== undefined) {
        const res = await api.delete(
          `/api/v1/jhas/${localStorage.getItem("fkJHAId")}/teams/${
            Teamform[index].id
          }/`
        );
      }
  ...
  const handelTeam = async (CreatedJhaId) => {
    for (let i = 0; i < Teamform.length; i++) {
      ...
      if (Teamform[i].id) {
        const res = await api
          .put(`${apiEndPoint}/${Teamform[i].id}/`, Teamform[i])
          .then()
  ...
  const checkList = async () => {
    ...
    const res = await api.get(
      `/api/v1/core/checklists/jha-safety-hazards/${projectId}/`
    );
  ...
  const checkListControl = async () => {
    ...
    const projectId = project.projectName.projectId;
    const res = await api.get(
      `/api/v1/core/checklists/jha-safety-ppe-checklist/${projectId}/`
    );
  ...
  const handelSelectOption = (checklistId, hazard) => {
    for (let i = 0; i <= formHazard.length; i++) {
      if (
        formHazard[i] != undefined &&
        formHazard[i]["hazard"] == hazard &&
        formHazard[i]["fkChecklistId"] == checklistId
      ) {
        return true;
      }
    }
  ...
  const handelSelectOptionPpe = (checklistId, safetyChecks) => {
    for (let i = 0; i <= formHazard.length; i++) {
      if (
        formHazard[i] != undefined &&
        formHazard[i]["safetyChecks"] == safetyChecks &&
        formHazard[i]["fkChecklistId"] == checklistId
      ) {
        return true;
      }
  ...
  const handleSubmitHazard = async (newJhaId) => {
    ...
    const resUpdate = await api
      .put(`/api/v1/jhas/${newJhaId}/bulkhazards/`, hazardUpdate)
                    ...
                    <Grid container spacing={3}>
                      ...
                      <Grid item md={6} xs={11}>
                        <TextField
                          ...
                          onChange={(e) =>
                            setForm({ ...form, jobOrderNumber: e.target.value })
                          }
                          fullWidth
                          variant="outlined"
                          className={classes.formControl}
                        ...
                        <MuiPickersUtilsProvider utils={DateFnsUtils}>
                          <KeyboardDatePicker
                            ...
                            onChange={(e) => {
                              setForm({
                                ...form,
                                jhaAssessmentDate: moment(e).format(
                                  "YYYY-MM-DD"
                                ),
                              });
                            }}
                    ...
                    <Grid container spacing={3}>
                      ...
                      <Grid item md={6} xs={11}>
                        <TextField
                          ...
                          value={
                            form.emergencyNumber ? form.emergencyNumber : ""
                          }
                          onChange={(e) =>
                            setForm({
                              ...form,
                      ...
                      <Grid item md={12} xs={12}>
                        <TextField
                          ...
                          onChange={(e) =>
                            setForm({
                              ...form,
                              evacuationAssemblyPoint: e.target.value,
                            })
                          }
                          fullWidth
                          variant="outlined"
                ...
                <Grid item md={12} sm={12} xs={12} className="paddTBRemove">
                  <Paper elevation={1} className="paperSection">
                    <Grid container spacing={3}>
                      ...
                      {Object.entries(checkGroupsControl).map(
                        ([key, value]) => (
                          <Grid item md={6}>
                            <FormControl component="fieldset">
                              <FormLabel component="legend">{key}</FormLabel>
                      ...
                      <Grid
                        ...
                        md={12}
                        xs={12}
                        className="formFieldBTNSection paddTRemove"
                        align="left"
                      >
                        <Button
                          variant="contained"
