...
const AssessmentAndDocument = (props) => {
  ...
  const handelActionTracker = async () => {
    ...
    const apiData = JSON.parse(localStorage.getItem("commonObject")).jha
      .assessmentIds;
  ...
  const handelJobDetailsDocument = async () => {
    ...
    const apiData = res.data.data.results;
    apiData.notifyTo == null
      ? (apiData.notifyTo = "")
      : (apiData.notifyTo = apiData.notifyTo.split(","));
    setFormDocument(apiData);

    setAdditionalJobDetails({
      ...additinalJobDetails,
      humanPerformanceAspects:
        apiData.humanPerformanceAspects !== null
          ? apiData.humanPerformanceAspects.split(",")
          : [],
      workStopCondition:
        apiData.workStopCondition !== null
          ? apiData.workStopCondition.split(",")
          : [],
  ...
  const handelPreformance = (e, value) => {
    if (e.target.checked === false) {
      const newData = additinalJobDetails.humanPerformanceAspects.filter(
        (item) => item !== value
      );
      setAdditionalJobDetails({
        ...additinalJobDetails,
      ...
      setAdditionalJobDetails({
        ...additinalJobDetails,
        humanPerformanceAspects: [
          ...additinalJobDetails.humanPerformanceAspects,
          value,
        ],
      });
    }
  ...
  const handelWorkDocument = (e, value) => {
    if (e.target.checked === false) {
      const newData = additinalJobDetails.workStopCondition.filter(
        (item) => item !== value
      );
      setAdditionalJobDetails({
        ...additinalJobDetails,
  ...
  const handelActionLink = () => {
    ...
    setProjectData({
      ...
      createdBy: userId,
      projectStructId: JSON.parse(localStorage.getItem("commonObject")).jha
        .projectStruct,
    });
  };
  ...
  const [submitLoaderDocument, setsubmitLoaderDocumentDocument] = useState(
    false
  );
  const ref = useRef();
  ...
  const handelNextDocument = async () => {
    ...
    if (
      ...
      data.append(
        "humanPerformanceAspects",
        additinalJobDetails.humanPerformanceAspects.toString()
      );
      ...
      if (
        additinalJobDetails.files &&
        additinalJobDetails.files !== undefined &&
        typeof additinalJobDetails.files !== "string"
      ) {
            ...
            <Grid item md={12} sm={12} xs={12} className="paddTBRemove">
              <Typography variant="h6" className="sectionHeading">
                <svg
                  ...
                  width="28.17"
                  height="28.45"
                  viewBox="0 0 28.17 28.45"
                >
                  <g
                    id="Group_5478"
                    data-name="Group 5478"
                    transform="translate(-1261.812 -224.131)"
                  >
                    <path
                      id="market-research-analysis"
                      d="M12.4,0A12.413,12.413,0,0,1,22.792,19.211l5.37,5.853-3.7,3.386-5.18-5.708A12.411,12.411,0,1,1,12.4,0Zm7.8,4.613a11.026,11.026,0,1,0,3.24,7.8A11.026,11.026,0,0,0,20.2,4.613Z"
                      transform="translate(1261.82 224.131)"
                      fill="#06425c"
                    />
                    <g
                      id="attention-signal-and-construction-worker-svgrepo-com"
                      transform="translate(1248.688 216.658)"
                    >
                      <path
                        id="iconos_43_"
                        d="M24.719,21.254h1.615l.308-2.512.09-.718.051-.384H24.271Zm-.167.41h2v2h-2Zm.9-8.624a.425.425,0,0,0-.359.205l-1.615,2.806-.231.4-.231.4L18.748,24.24a.406.406,0,0,0-.051.205.334.334,0,0,0,.051.192.409.409,0,0,0,.359.205H31.819a.4.4,0,0,0,.41-.41.443.443,0,0,0-.09-.256l-6.33-10.931A.409.409,0,0,0,25.45,13.04ZM28.115,24.06h-8.3l4.165-7.2.231-.4.231-.4,1.038-1.781,5.651,9.778Z"
                        fill="#06425c"
                      />
                    </g>
                  </g>
                    ...
                    <Grid item md={12} xs={12}>
                      ...
                      {false ? (
                        <Grid
                          item
                          xs={12}
                          className="formFieldBTNSection paddTRemove marginB15"
                          align="left"
                        >
                          <Button
                            variant="contained"
                        ...
                        <FormGroup>
                          {document.map((option) => (
                            <FormControlLabel
                              ...
                              onChange={async (e) =>
                                handelWorkDocument(e, option.inputValue)
                              }
                            />
                          ))}
                        </FormGroup>
                              ...
                              <FormGroup>
                                {value.map((option) => (
                                  <FormControlLabel
                                    ...
                                    label={option.inputLabel}
                                    checked={additinalJobDetails.humanPerformanceAspects.includes(
                                      option.inputValue
                                    )}
                                    onChange={async (e) =>
                                      handelPreformance(e, option.inputValue)
                                    }
                                  />
                                ))}
                              </FormGroup>
                              ...
                              {notificationSentValue.length != 0
                                ? notificationSentValue.map((value) => (
                                    <FormControlLabel
                                      className="selectLabel"
                                      control={
                                        <Checkbox name={value.roleName} />
                                      }
                                      label={value.roleName}
                                      checked={
                                        formDocument.notifyTo &&
                                        formDocument.notifyTo !== null &&
                                        formDocument.notifyTo.includes(
                                          value.id.toString()
                                        )
                                      }
                                      onChange={async (e) =>
