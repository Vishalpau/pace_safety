...
function ObservationBulkupload() {
  ...
  const handleBulkUploadfilePush = () => {
    history.push("/app/icare-bulkuploadfile");
  };
  ...
  const refreshButton = async (id) => {
    // console.log(id);
    document.getElementById(`button${id}`).innerHTML = "Loading";
    const res = await appapi.get("api/v1/core/bulkuploads/");
    let resData = res.data.data.results.results;
    console.log(resData.filter((one) => one.id === id));
    let results = data.map((x) => {
      let el = resData.find((y) => y.id === x.id);
      if (el)
        return {
          ...x,
    ...
    document.getElementById(`button${id}`).innerHTML = "Refresh";
  };
                ...
                <div className="bulkUploadSection">
                  <Grid container spacing={3}>
                    <Grid
                      ...
                      xs={12}
                      align="right"
                      className={classes.bulkBTNTopBox}
                    >
                      <Button
                        variant="contained"
                        size="small"
                      ...
                      {data.map((row) => (
                        <TableRow>
                          ...
                          <TableCell>
                            <button
                              id={`button${row.id}`}
                              onClick={() => refreshButton(row.id)}
                              disabled={row.uploadStatus === "Completed"}
                              title="Refresh"
                              style={{padding: "8px 10px", background: "#dedede", fontSize: "12px"}}
                            >
                              Refresh
                            </button>
                          </TableCell>
