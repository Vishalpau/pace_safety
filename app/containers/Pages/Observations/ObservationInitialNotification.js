...
const ObservationInitialNotification = (props) => {
  ...
  const userDetails =
    JSON.parse(localStorage.getItem("userDetails")) !== null
      ? JSON.parse(localStorage.getItem("userDetails"))
      : null;
  const userCompany = userDetails.companies.filter(
    (company) => company.companyId === fkCompanyId
  );
  const userDepartment = userCompany[0].departments.filter(
    (userDepartment) => userDepartment.fkUserId === userId
  );
  const project =
  ...
  const [form, setForm] = useState({
    ...
    reportedByDepartment: userDepartment[0]
      ? userDepartment[0].departmentName
      : "",
  ...
  const handelTime = (value) => {
    ...
    const requireTime = moment(noGmt)
      .format()
      .toString();
    const observedAtTime = requireTime.replace("+05:30", ".000Z");
  ...
  const handleSubmit = async () => {
    ...
    form.notifyToOther = notifyToOther;
            ...
            const response = api
              ...
              .then((res) => {
                if (res.status === 200 || res.status === 201) {
                  const notificationSent = api.get(
                    `/api/v1/observations/${localStorage.getItem(
                      "fkobservationId"
                    )}/sentnotification/`
                  );
  ...
  const handleNotify = async (value, index, e) => {
    if (e.target.checked === true) {
      ...
      const temp = [...notifyToList];
      const newData = temp.filter((item) => item !== value);
  ...
  const fetchTags = async () => {
    ...
    const sorting = temp.sort((a, b) => a.id - b.id);
  ...
  const handleBreakdown = async (e, index, label, selectvalue) => {
    ...
    const tempDepthAndId = selectDepthAndId;
    const dataDepthAndId = tempDepthAndId.filter(
  ...
  const handlechange = (name) => {
    const names = companyUserDetails.filter((user) => name.includes(user.name));
    setNotifyToOther(names[0].id + "-" + names[0].email);
  };
  ...
  const handleIndiv = async () => {
    ...
    const res = await api(config);
    let data = res.data.data.results.users;
    setCompanyUserDetails(data);
    data = data.map((cd) => cd.name);
    setCompanyUser(data);
  };
                      ...
                      <Grid item md={6} xs={12} className={classes.formBox}>
                        <MuiPickersUtilsProvider utils={MomentUtils}>
                          <KeyboardDateTimePicker
                            ...
                            helperText={
                              error.observedAt ? error.observedAt : null
                            }
                      ...
                      <Grid item md={12} xs={12} className={classes.formBox}>
                        <TextField
                          ...
                          onChange={(e) => {
                            setForm({
                              ...form,
                              observationTitle: e.target.value,
                            });
                          }}
                        />
                      </Grid>
                        ...
                        >
                          ...
                          <RadioGroup
                            ...
                            onChange={(e) => {
                              setForm({
                                ...form,
                                isSituationAddressed: e.target.value,
                              });
                            }}
                          >
                    ...
                    <Grid container spacing={3}>
                        ...
                        <FormControl component="fieldset">
                          ...
                          <RadioGroup
                            ...
                            onChange={(e) => {
                              setForm({
                                ...form,
                                personRecognition: e.target.value,
                              });
                            }}
                          >
                            {radioSituation.map((value) => (
                      ...
                      <Grid item md={12} xs={12} className={classes.formBox}>
                        <FormLabel
                          ...
                          component="legend"
                        >
                          Do you need to escalate the issue to Individual?
                        </FormLabel>
                        <Autocomplete
                          // multiple
                          className="marginT15"
                        ...
                        <Grid item md={12} xs={12} className={classes.formBox}>
                          <FormControl component="fieldset">
                            <FormLabel
                              ...
                              className="checkRadioLabel"
                            >
                              Confirm if the foreman was present at the time of
                              your observation?
                            </FormLabel>
                            <RadioGroup
                              row
