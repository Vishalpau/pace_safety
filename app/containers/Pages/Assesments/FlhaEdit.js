...
const FlhaDetails = () => {
  ...
  const taskUrl = `/api/v1/flhas/${flhaNumber}/criticaltasks/`;
  const radioDecide = ["Yes", "No", "N/A"];
  const visualType = ["Site pictures", "Team pictures", "Tools and tackles"];
  const history = useHistory();

  const colorSeverityObj = {
    "Slightly harmful": 2,
    Harmful: 4,
    "Very harmful": 6,
    "Extremely harmful": 8,
  ...
  const fetchFlhaData = async () => {
    ...
    const flhaData = res.data.data.results;
    flhaData.notifyTo === null
      ? (flhaData.notifyTo = [])
      : (flhaData.notifyTo = flhaData.notifyTo);
    await setFlhaForm(flhaData);
    handelProjectStuctId(
      flhaData.fkCompanyId,
      flhaData.fkProjectId,
      flhaData.fkProjectStructureIds
    );
  };

  const fetchJobVisualData = async () => {
    const res = await api.get(
      `/api/v1/flhas/${flhaNumber}/visualconfirmations/`
    );
    const jobVisualData = res.data.data.results;
  ...
  const handelTaskPerVersion = (data) => {
    ...
    if (version == "") {
      handelTaskColor(
        dataVersion[
          Object.keys(dataVersion)[Object.keys(dataVersion).length - 1]
        ]
      );
      setVersionSelected(
        Object.keys(dataVersion)[Object.keys(dataVersion).length - 1]
  ...
  const handelVersionShow = (version = "") => {
    const dataVersion = taskPerVersion[version];
  ...
  const handelTaskColor = (tasksData) => {
    if (tasksData !== undefined && tasksData.length > 0) {
      for (let i = 0; i < tasksData.length; i++) {
        const hazardData = tasksData[i].hazards;
        for (let j = 0; j < hazardData.length; j++) {
          ...
          if (hazardData[j].riskRatingLevel !== "") {
            if (
              hazardData[j].riskRatingLevel === "2 Trivial" ||
              hazardData[j].riskRatingLevel === "4 Trivial"
            ) {
              hazardData[j].riskRatingColour = "#1EBD10";
            } else if (
              hazardData[j].riskRatingLevel === "6 Tolerable" ||
              hazardData[j].riskRatingLevel === "8 Tolerable"
            ) {
              hazardData[j].riskRatingColour = "#008000";
            } else if (
              hazardData[j].riskRatingLevel === "12 Moderate" ||
              hazardData[j].riskRatingLevel === "16 Moderate"
            ) {
              hazardData[j].riskRatingColour = "#F3C539";
            } else if (
              hazardData[j].riskRatingLevel === "18 Substantial" ||
              hazardData[j].riskRatingLevel === "24 Substantial"
            ) {
  ...
  const handelTaskData = async (version = "") => {
    const taskData = await api.get(`${taskUrl}`);
    console.log(taskData, "cccc");
    const task = taskData.data.data.results;
    const tasksData = task.tasks;
    handelTaskPerVersion(task);
    const allVersion = task.versions;
  ...
  const handelProjectStuctId = async (companyId, projectId, projectStruct) => {
    const breakDownUrl = [];
    const allProjetcData = JSON.parse(localStorage.getItem("userDetails"))
      .companies;
    const flhaCompany = allProjetcData.filter(
      (compamyData) => compamyData.companyId == companyId
    );
    const flhaProject = flhaCompany[0].projects.filter(
      (projectData) => projectData.projectId == projectId
    );
    const { projectName } = flhaProject[0];
    const allBreakDownValues = flhaProject[0].breakdown;
    const breakDown = allBreakDownValues.filter((breakDownStruct) =>
      breakDownUrl.push(breakDownStruct.structure[0].url)
    );
    setProjectData({ ...projectData, projectName });
  };
  const pickListValuesData = JSON.parse(localStorage.getItem("pickList"));
  console.log(pickListValuesData);

  const handelPickListValues = async () => {
    const reasonReportingPickList = await pickListValuesData["84"];
    // console.log(reasonReportingPickList,'reasonReporting');
    const hazardPickList = await pickListValuesData["83"];
    const permitTypeCheckList = await pickListValuesData["81"];
    await setPickListValues({
      resonReporting: reasonReportingPickList,
      hazard: hazardPickList,
      permitType: permitTypeCheckList,
    });
  };
  ...
  const handelHazard = (index, indexHazard, type, value) => {
    const temp = [...taskForm];
    const taskHazard = temp[index].hazards[indexHazard];
  ...
  const handleRiskChange = (e, key, fieldname, indexHazard) => {
    const temp = [...taskForm];
    const taskHazard = temp[key].hazards;
    const txt = e.nativeEvent.target.innerText;
    taskHazard[indexHazard][fieldname] = e.target.value;
    const riskSeverity =
      taskHazard[indexHazard].riskSeverityValue == undefined ||
      taskHazard[indexHazard].riskSeverityValue == "" ||
      isNaN(taskHazard[indexHazard].riskSeverityValue)
        ? 1
        : taskHazard[indexHazard].riskSeverityValue;
    const riskProbability =
      taskHazard[indexHazard].riskProbabilityValue == undefined ||
      taskHazard[indexHazard].riskProbabilityValue == "" ||
      isNaN(taskHazard[indexHazard].riskProbabilityValue)
        ? 1
        : taskHazard[indexHazard].riskProbabilityValue;
  ...
  const handelVisualConfirmation = (index, value) => {
    const temp = [...jobVisualConfirmation];
    if (value === "Yes") {
  ...
  const handelJobVisualAttachment = (index) => {
    const confirmationStatus =
      jobVisualConfirmation[index].visualConfirmationStatus;
  ...
  const handelVisualTaskSubmit = async () => {
    for (const key in jobVisualConfirmation) {
      typeof jobVisualConfirmation[key].visualConfirmationAttachment ===
        "string" &&
        delete jobVisualConfirmation[key].visualConfirmationAttachment;
      const jobVisualizationData = new FormData();
  ...
  const handelTaskSubmit = async () => {
    ...
    taskForm.map((value, index) => {
      ...
      const hazard = value.hazards;
      hazard.length > 0 &&
        hazard.map((valueHazard, indexHazard) => {
    ...
    const res = await api
      .post(`/api/v1/flhas/${flhaNumber}/criticaltasks/`, taskForm)
      .then(() => handelVisualTaskSubmit())
      .catch();
  };
  ...
  const handelFlhaSubmit = async () => {
    ...
    flhaForm.notifyTo.length == 0 ||
    flhaForm.notifyTo == "null" ||
    flhaForm.notifyTo == null
      ? (flhaForm.notifyTo = "null")
      : (flhaForm.notifyTo = flhaForm.notifyTo.toString());
    const flhaData = new FormData();
    Object.entries(flhaForm).map(([key, value]) => {
      if (value !== "" && value !== null) {
        flhaData.append(key, value);
      }
    });
    if (flhaForm.files !== null && flhaForm.files !== undefined) {
      flhaForm.files.map((file) => {
        flhaData.append("files", file);
      });
    }
    const res = await api
      .put(`/api/v1/flhas/${flhaNumber}/`, flhaData)
      .then(() => handelTaskSubmit())
      .catch();
    // handelTaskSubmit()
  };
          ...
          {isLoading == false ? (
            <Grid container spacing={3}>
              ...
              <Grid
                ...
                sm={12}
                xs={12}
                className="marginT10 paddBRemove"
              >
                <Paper elevation={1} className="paperSection blockBGColor">
                  <Grid container spacing={3}>
                    <Grid item md={3} sm={3} xs={12} className="formWithBG">
                      ...
                      >
                        ...
                        <Select
                          ...
                          defaultValue=""
                        >
                          {pickListValues.resonReporting !== undefined &&
                            pickListValues.resonReporting.map(
                              (selectValues) => (
                                <MenuItem
                    ...
                    <Grid
                      ...
                      xs={12}
                      align="right"
                      className="reviseCount"
                    >
                      <List
                        component="nav"
                        className="inlineListBTN"
                        aria-label="main mailbox folders"
                      >
                        <ListItem
                          button
                          className={versionSelected == 1.0 ? "active" : ""}
                        >
                          <ListItemText
                            primary="Initial assessment"
                            onClick={(e) => handelVersionShow("1.0")}
                          />
                        </ListItem>
                        ...
                        {version.slice(1, version.length).map((value) => (
                          <ListItem
                            button
                            className={versionSelected == value ? "active" : ""}
                          >
                            <ListItemText
                              primary={`X-FLHA ${value}`}
                              onClick={(e) => handelVersionShow(value)}
                            />
                          </ListItem>
                        ))}
                        ...
                        {false && (
                          <ListItem button className="btnNoneLink">
                            <ListItemText
                              onClick={handleClickOpenRevision("paper")}
                            ...
                            <TableContainer>
                              ...
                              >
                                <TableHead className={classes.tableHeading}>
                                  <TableRow
                                    className={classes.cellHeightCustom}
                                  >
                                    <TableCell
                                      align="left"
                                ...
                                <TableBody>
                                  ...
                                  >
                                    <TableCell align="left">
                                      <Link to="">Initial assessment</Link>
                                    </TableCell>
                                    <TableCell align="left">
                                      Initial assessment
                                    </TableCell>
                ...
                <>
                            ...
                            >
                              ...
                              >
                                {departments !== undefined &&
                                  departments.map((selectValues) => (
                                    <MenuItem
                                      value={selectValues}
                                      onClick={(e) =>
                                        setFlhaForm({
                                          ...flhaForm,
                                          department: selectValues,
                                        })
                                      }
                                    >
                                      {selectValues}
                    ...
                    <Typography variant="h6" className="sectionHeading">
                      ...
                      >
                        <g
                          ...
                          transform="translate(-327 -742)"
                        >
                          <g
                            id="Group_4995"
                            data-name="Group 4995"
                            transform="translate(327 742)"
                          >
                            <g id="outline-assignment-24px">
                              <g id="Bounding_Boxes">
                  ...
                  <Grid item md={12} sm={12} xs={12} className="paddTBRemove">
                    <Grid container spacing={3}>
                      <Grid
                        ...
                        sm={12}
                        xs={12}
                        className="sectionSubHeading paddBRemove"
                      >
                        <div className="subsectiontitle">
                          <Typography variant="h6" className="sectionHeading">
                            <svg
                              id="Group_5073"
                              data-name="Group 5073"
                              xmlns="http://www.w3.org/2000/svg"
                              width="40"
                              height="40"
                              viewBox="0 0 40 40"
                      ...
                      >
                              ...
                              >
                                ...
                                >
                                  <Typography className={classes.heading}>
                                    Task#
                                    {index + 1} -
                                    {taskForm[index].taskIdentification}
                                  </Typography>
                                  <Grid container justify="flex-end">
                                    <Button>
                                      <DeleteForeverOutlinedIcon
                                        onClick={(e) => handelTaskRemove(index)}
                                      />
                                    </Button>
                                  </Grid>
                                </AccordionSummary>
                                              ...
                                              <Grid container spacing={3}>
                                                ...
                                                <Grid item sm={4} xs={8}>
                                                  ...
                                                  >
                                                    ...
                                                    >
                                                      {pickListValues.hazard !==
                                                        undefined &&
                                                        pickListValues.hazard.map(
                                                          (selectValues) => (
                                                            <MenuItem
                                                              value={
                                                                selectValues.value
                                                    ...
                                                    >
                                                      ...
                                                      >
                                                        ...
                                                        >
                                                          ...
                                                          >
                                                            ...
                                                            <Select
                                                              ...
                                                              label="Incident Type"
                                                              value={
                                                                valueHazard.riskProbabilityValue
                                                              }
                                                              onChange={(e) =>
                                                                handleRiskChange(
                                                                  e,
                                                        ...
                                                        <Grid
                                                          ...
                                                          style={{
                                                            backgroundColor:
                                                              valueHazard.riskRatingColour,
                                                            marginTop: "10px",
                                                          }}
                                                        >
                                                          {valueHazard.riskRatingLevel
                                                            ? `${
                                                                valueHazard.riskRatingLevel
                                                              } risk`
                                                            : ""}
                        ...
                        <Grid
                          item
                          xs={12}
                          className="formFieldBTNSectionAdd"
                          align="left"
                        >
                          <Button
                            variant="contained"
                                  ...
                                  <TableRow className={classes.cellHeight}>
                                          ...
                                          >
                                            {radioDecide.map((value) => (
                                              <FormControlLabel
                                                ...
                                                label={value}
                                                checked={
                                                  jobVisualConfirmation[index]
                                                    .visualConfirmationStatus ===
                                                  value
                                                }
                                                onChange={(e) =>
                                                  handelVisualConfirmation(
                                                    index,
                                                    value
                                                  )
                                                }
                                    ...
                                    <TableCell align="left">
                                      <input
                                        ...
                                        onChange={(e) => {
                                          handelVisualAttachment(
                                            index,
                                            e,
                                            "visualConfirmation"
                                          );
                                        }}
                                        disabled={handelJobVisualAttachment(
                                          index
                                        )}
                                      />
                                      ...
                                      null ? null : typeof jobVisualConfirmation[
                                          index
                                        ].visualConfirmationAttachment ===
                                        "string" ? (
                                        <Attachment
                            ...
                            >
                              {radioDecide.map((value) => (
                                <FormControlLabel
                                  ...
                                  label={value}
                                  checked={
                                    flhaForm.permitToWork.toLowerCase() ==
                                    value.toLowerCase()
                                  }
                                  onChange={(e) =>
                                    setFlhaForm({
                        ...
                        <Grid item md={5} xs={12}>
                          <FormControl component="fieldset">
                            <FormLabel
                              ...
                              className="checkRadioLabel"
                            >
                              *Is a First Aid/Medical Aid present for your
                              shift?
                            </FormLabel>
                            <RadioGroup
                              row
                            ...
                            >
                              {radioDecide.slice(0, 2).map((value) => (
                                <FormControlLabel
                                  ...
                                  label={value}
                                  checked={
                                    flhaForm.accessToJobProcedure == value
                                  }
                                  onChange={(e) =>
                                    setFlhaForm({
                                      ...flhaForm,
                                      accessToJobProcedure: value,
                                    })
                                  }
                                />
                              ))}
                  ...
                  >
                    ...
                    <Button
                      ...
                      onClick={(e) =>
                        history.push(
                          `/app/pages/assesments/FlhaSummary/${flhaNumber}`
                        )
                      }
                    >
                      Cancel
