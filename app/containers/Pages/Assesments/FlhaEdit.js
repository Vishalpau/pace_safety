...
import {
  companyId, HEADER_AUTH, projectId, SSO_URL, userId
...
const FlhaDetails = () => {
  ...
  const handelProjectStuctId = async (companyId, projectId, projectStruct) => {
    const breakDownUrl = [];
    const allProjetcData = JSON.parse(localStorage.getItem('userDetails')).companies;
    const flhaCompany = allProjetcData.filter(compamyData => compamyData.companyId == companyId);
    const flhaProject = flhaCompany[0].projects.filter(projectData => projectData.projectId == projectId);
    const { projectName } = flhaProject[0];
    const allBreakDownValues = flhaProject[0].breakdown;
    const breakDown = allBreakDownValues.filter(breakDownStruct => breakDownUrl.push(breakDownStruct.structure[0].url));
  ...
  const handelJobVisualAttachment = (index) => {
    const confirmationStatus = jobVisualConfirmation[index].visualConfirmationStatus;
  ...
  const handelTaskSubmit = async () => {
    const taskFields = ['riskProbabilityValue', 'riskSeverityValue', 'riskRatingColour', 'fkTaskId', 'updatedAt', 'updatedBy', 'createdAt'];
                    ...
                    <Grid item md={12} sm={12} xs={12} className="paddTBRemove">
                      <Paper elevation={1} className="paperSection">
                        <Grid container spacing={3}>
                          ...
                          <Grid item xs={2} className="formFieldBTNSection" align="center"><Button variant="outlined" disabled onClick={handleClickOpen('paper')}>Select job </Button></Grid>
...
Hazard#
                                            ...
                                            {' '}
-
                                            {taskForm[index].hazards[indexHazard].hazards}
                                          </Typography>
                                          <Button>
                                                ...
                                                <RadioGroup row aria-label="hazardpresent" name="hazardpresent">
                                                  {radioDecide.map((value) => (
                                                    <FormControlLabel
                                                      ...
                                                      onChange={(e) => handelHazard(index, indexHazard, 'hazardPresnt', value)
                                                      }
                                                    />
                                                  ))}
                                                </RadioGroup>
                                                      ...
                                                      <RadioGroup row aria-label="hazardpresent" name="hazardpresent">
                                                        {radioDecide.map((value) => (
                                                          <FormControlLabel
                                                            ...
                                                            onChange={(e) => handelHazard(index, indexHazard, 'controlPut', value)
                                                            }
                                                          />
                                                        ))}
                                                      </RadioGroup>
...
Additional information
                    ...
                    <Grid item md={12} sm={12} xs={12} className="buttonActionArea">
                      ...
                      <Button
                        ...
                        onClick={(e) => history.push(`/app/pages/assesments/FlhaSummary/${flhaNumber}`)}
                      >
                    Cancel
                      </Button>
                    </Grid>
