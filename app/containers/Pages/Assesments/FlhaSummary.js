...
class SimpleTabs extends React.Component {
  ...
  handleProjectName = (flha) => {
    ...
    if (Object.keys(flha).length > 0) {
      let projectData = companies
        .filter((company) => company.companyId == flha.fkCompanyId)[0]
        .projects.filter((project) => project.projectId === flha.fkProjectId)[0]
        .projectName;
      return projectData;
    }
    return "";
  ...
  render() {
    ...
    return (
      <CustomPapperBlock
        title={"FLHA Number:" + flha.flhaNumber}
          ...
          <Grid item md={9} xs={12}>
            ...
            <Grid container spacing={3}>
              <Grid item md={12} xs={12}>
                <List
                  component="nav"
                  className="inlineListBTN versionTabBox"
                  aria-label="main mailbox folders"
                >
                  <ListItem
                    button
                    className={
                      this.state.selectedVersion == 1.0 ? "active" : ""
                    }
                  >
                    <ListItemText
                      primary="Initial assessment"
                      onClick={(e) => this.getPreventiveControls("1.0")}
                    />
                  </ListItem>
                  {this.state.versions !== undefined &&
                    this.state.versions.length > 0 &&
                    this.state.versions
                      .slice(1, this.state.versions.length)
                      .map((version) => (
                        <ListItem
                          button
                          className={
                ...
                <TabContainer className={classes.paddZero}>
                  <ViewHazard
                    criticalTasks={this.state.criticalTasks}
                    visualConfirmations={this.state.visualConfirmations}
                    flha={this.state.flha}
                  />
                </TabContainer>
              </Grid>
            </Grid>
