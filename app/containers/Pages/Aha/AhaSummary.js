...
function AhaSummary(props) {
  ...
  //   history.push("/app/pages/aha/aha-comment");
  // };

  const handleActivityPush = async () => {
    history.push("/app/pages/aha/aha-activity");
  };
  ...
  const assessmentDataValues = async () => {
    ...
    const documentCondition = await api.get(
      `${baseUrl}/api/v1/core/checklists/aha-document-conditions/${projectId}/`
    );
  ...
  const handelLessionLearnedChanges = (side) => {
    if (
      ...
      if (
        ...
        setMessageSnackbar(
          `${errorMessage} ${errorAssessment} , ${errorApproval}`
        );
        handleClickSnackBar();
      } else if (
        (ahaData.notifyTo !== null && ahaData.wrpApprovalUser == null) ||
        ahaData.sapApprovalUser == null
      ) {
  ...
  const fetchHzardsData = async () => {
    ...
    for (var i = 0; i < result.length; i++) {
      if (result[i].riskRating !== "") {
        if (
          result[i].riskRating === "2 Trivial" ||
          result[i].riskRating === "4 Trivial"
        ) {
          colorAssessmment[i].riskRatingColour = "#009933";
        } else if (
          result[i].riskRating === "6 Tolerable" ||
          result[i].riskRating === "8 Tolerable"
        ) {
          colorAssessmment[i].riskRatingColour = "#8da225";
        } else if (
          result[i].riskRating === "12 Moderate" ||
          result[i].riskRating === "16 Moderate"
        ) {
          colorAssessmment[i].riskRatingColour = "#FFBF00";
        } else if (
          result[i].riskRating === "18 Substantial" ||
          result[i].riskRating === "24 Substantial"
        ) {
  ...
  const handleProjectName = (projectId) => {
    ...
    const fetchCompany = userName.filter(
      (user) => user.companyId === ahaData.fkCompanyId
    );
                      ...
                      >
                        <ul className="SummaryTabList">
                          ...
                          <li>
                            <Button
                              color={
                                approvals == true ? "secondary" : "primary"
                              }
                              variant={
                                ahaData.wrpApprovalUser !== null &&
                                ahaData.sapApprovalUser !== null
