...
function Aha(props) {
  ...
  const handleSummaryPush = async (index) => {
    const { id } = allAHAData[index];
    const { fkProjectStructureIds } = allAHAData[index];
  ...
  const fetchAllAHAData = async () => {
    ...
    const res = await api.get(
      `api/v1/ahas/?search=${searchIncident}&companyId=${fkCompanyId}&projectId=${fkProjectId}&projectStructureIds=${fkProjectStructureIds}`
    );
  ...
  const handleChange = async (event, value) => {
    ...
    const fkProjectStructureIds = struct.slice(0, -1);
    const res = await api.get(
      `api/v1/ahas/?search=${searchIncident}&companyId=${fkCompanyId}&projectId=${fkProjectId}&projectStructureIds=${fkProjectStructureIds}&page=${value}`
    );
